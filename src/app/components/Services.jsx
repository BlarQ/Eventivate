...
const Services = () => {
  return (
    <div className='h-[60vh] px-5 sm:px-12'>
        <h1 className='text-3xl font-bold text-center py-4'>What We Offer</h1>

        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 p-5'>
          {
            services.map((item) => {
              return (
                <div key={item.id} className='flex flex-col group gap-4 p-5 border rounded-2xl shadow'>
                  <div className='group-hover:scale-[1.01] duration-300'>

                  <Image className='w-full h-full group-hover:scale-[1.01] duration-300 rounded-2xl aspect-square' loading='lazy' src={item.img} alt={item.title} width={300} height={300} />
                  </div>
